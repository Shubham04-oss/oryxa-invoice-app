...
/**
 * Format currency for PDF (ASCII-safe)
 * Replaces Unicode currency symbols with ASCII text
 */
...
export async function generateInvoicePDF(invoice, tenant) {
  ...
  for (const item of invoice.items) {
    const itemTotal = formatCurrencyForPDF(item.total, invoice.currency);
    const unitPrice = formatCurrencyForPDF(item.unitPrice, invoice.currency);
  ...
  page.drawText(formatCurrencyForPDF(invoice.subtotal, invoice.currency), {
    x: 490,
    y,
    size: fontSize,
    font,
  ...
  if (invoice.discountTotal > 0) {
    ...
    page.drawText(`-${formatCurrencyForPDF(invoice.discountTotal, invoice.currency)}`, {
      x: 490,
      y,
      size: fontSize,
      font,
  ...
  page.drawText(formatCurrencyForPDF(invoice.taxTotal, invoice.currency), {
    x: 490,
    y,
    size: fontSize,
    font,
  ...
  page.drawText(formatCurrencyForPDF(invoice.total, invoice.currency), {
    x: 490,
    y,
    size: fontSize + 2,
    font: fontBold,
